    ...
    document.querySelector('#btnRandom').addEventListener('click', () => {
        ...
        if (selectedMember.length > 0) {
            ...
            for(var i = 0; i < selectMemLength; i++) {
                ...
                html.push(`<img src="${selectedMember[randomIndex].image}" alt="${selectedMember[randomIndex].mem}" class="rounded-full border-2 border-blue-500 mb-2 w-[100px] h-[100px]  " />`);
                html.push(`<span class="text-black">${i + 1}. ${selectedMember[randomIndex].mem}</span>`);
                html.push(`</div>`);
